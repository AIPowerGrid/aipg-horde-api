...
<a href="/terms">Terms of Service</a>
    ...
    # Get available models (with error handling) - temporarily disabled
    # try:
    ...
    #         image_models_list += f"• {model['name']} ({model['count']} threads, {model['queued']} queued)\n"
    #     if len(image_models) > 10:
    #         image_models_list += f"*... and {len(image_models) - 10} more models*\n"
    #     
    #     text_models_list = ""
    #     for model in text_models[:10]:  # Show top 10
    #         text_models_list += f"• {model['name']} ({model['count']} threads, {model['queued']} queued)\n"
    #     if len(text_models) > 10:
    #         text_models_list += f"• {model['name']} ({model['count']} threads, {model['queued']} queued)\n"
    #     if len(text_models) > 10:
    #         text_models_list += f"*... and {len(text_models) - 10} more models*\n"
    #     
    #     # Get top models by queue
    #     top_models = sorted(available_models, key=lambda x: x.get('queued', 0), reverse=True)[:5]
    #     top_models_list = ""
    #     for model in top_models:
    #         top_models_list += f"• {model['name']} ({model['queued']} queued, {model['count']} threads)\n"
    # except Exception as e:
    #     # Fallback if models can't be loaded
    #     image_models_list = "• Models loading...\n"
    #     text_models_list = "• Models loading...\n"
    #     top_models_list = "• Models loading...\n"
    #     image_models = []
    #     text_models = []
